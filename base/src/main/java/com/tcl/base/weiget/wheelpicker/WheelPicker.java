...
public class WheelPicker extends View implements IWheelPicker, Runnable {
    ...
    public WheelPicker(Context context, AttributeSet attrs) {
        ...
        mIndicatorSpace = a.getDimensionPixelSize(R.styleable.WheelPicker_wheel_indicator_space, 0);
    ...
    private void computeTextSize() {
        ...
        if (hasSameWidth) {
            mTextMaxWidth = (int) mPaint.measureText(convertItemShowTxt(mData.get(0)));
        } else if (isPosInRang(mTextMaxWidthPosition)) {
            mTextMaxWidth = (int) mPaint.measureText
                    (convertItemShowTxt(mData.get(mTextMaxWidthPosition)));
    ...
    private void computeIndicatorRect() {
        ...
        int halfIndicatorHeight = mIndicatorSpace / 2;
        int indicatorHeadCenterY = mWheelCenterY + mHalfItemHeight;
        int indicatorFootCenterY = mWheelCenterY - mHalfItemHeight;
        if (halfIndicatorHeight > 0) {
            indicatorHeadCenterY = mWheelCenterY + halfIndicatorHeight;
            indicatorFootCenterY = mWheelCenterY - halfIndicatorHeight;
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int drawnDataPos = drawnDataStartPos + mSelectedItemPosition,
            ...
            if (isCyclic) {
                ...
                data = convertItemShowTxt(mData.get(actualPos));
            } else {
                if (isPosInRang(drawnDataPos))
                    data = convertItemShowTxt(mData.get(drawnDataPos));
            }
